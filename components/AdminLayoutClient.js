    ...
    subItems: [
      {
        ...
        required: ["SETTINGS_READ", "SETTINGS_WRITE"],
      },
      {
      ...
      },
      {
        text: "Test & Parameter",
        path: "/settings/tests",
        required: ["TEST_READ", "TEST_WRITE"],
      },
      {
        text: "PDF Frame Setting",
        path: "/settings/pdf",
        required: ["SETTINGS_READ", "SETTINGS_WRITE"],
      },
      {
...
const getExpiryMessage = (expireAt) => {
  ...
  const diffDays = Math.round(
    (expiryDateOnly - nowDateOnly) / oneDayMs
  );
...
export default function AdminLayoutClient({ admin, children }) {
  ...
  /*
   * Simple drawer values
   */
  const isDrawerExpanded = !isMdUp || desktopOpen;

  const currentDrawerWidth =
    isMdUp && !desktopOpen ? 72 : drawerWidth;
  ...
  /*
   * Collapsed desktop submenu hover
   */
  ...
  useEffect(() => {
    ...
    if (cleanPath === "/" || cleanPath === "/dashboard") {
      ...
      hasAccess =
        userPerms.includes("REGISTRATION_READ") ||
        userPerms.includes("REGISTRATION_WRITE");
      ...
      hasAccess =
        userPerms.includes("DOCTOR_READ") ||
        userPerms.includes("DOCTOR_WRITE");
    } else if (cleanPath.startsWith("/members")) {
      hasAccess =
        userPerms.includes("MEMBER_READ") ||
        userPerms.includes("MEMBER_WRITE");
    } else if (cleanPath.startsWith("/settings")) {
      hasAccess =
        userPerms.includes("SETTINGS_READ") ||
        userPerms.includes("SETTINGS_WRITE") ||
        userPerms.includes("TEST_READ") ||
        userPerms.includes("TEST_WRITE");
    }
      ...
      .map((item) => {
        ...
        return {
          ...
          subItems: item.subItems.filter(
            (subItem) =>
              !subItem.required ||
              hasPermission(subItem.required)
          ),
        };
      });
  }, [admin]);
   ...
   */
  const getPageTitle = () => {
    const matched = STATIC_MENU_ITEMS.find(
          ...
          >
            ...
            <Box
              sx={{
                ...
                transition:
                  "transform 0.24s cubic-bezier(0.4, 0, 0.2, 1), visibility 0.24s ease",
