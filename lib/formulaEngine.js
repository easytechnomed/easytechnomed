...
export function determineFlag(value, thresholds, param = null) {
  ...
  const hasNumericThresholds = thresholds && (
  ...
  const isNormalKeyword = [
    "negative", "non-reactive", "non reactive", "nonreactive", "absent",
    "not detected", "not-detected", "nil", "normal", "clear", "not seen",
    "non-immune", "nonimmune"
  ...
  const isAbnormalKeyword = [
    "positive", "reactive", "present", "detected", "abnormal", "trace", "seen",
    "+", "++", "+++", "++++", "1+", "2+", "3+", "4+", "cloudy", "turbid", "hazy"
  ].some(term => {
    if (term === "reactive") {
    ...
    return valLower === term || (term.startsWith("+") && valLower.includes(term));
  });
    ...
    for (const form of formulasToRun) {
      ...
      if (canEval) {
        ...
        if (result !== null && !isNaN(result)) {
          ...
          // Save/update PatientResult by direct ID or create to prevent MySQL lock timeouts
          const existingResult = (registration.results || []).find(r => r.testParameterId === form.outputTestParameterId);
          if (existingResult) {
            await db.patientResult.update({
            ...
            existingResult.value = String(roundedResult);
            existingResult.flag = flag;
          } else {
            const created = await db.patientResult.create({
              data: {
                registrationId,
            ...
            if (!registration.results) registration.results = [];
            registration.results.push(created);
          }
