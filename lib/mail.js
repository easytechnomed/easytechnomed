...
export async function sendPlanRenewedEmail({
  ...
  console.log(
    `[MAIL_DEBUG] Sending sendPlanRenewedEmail to: ${email}, workspace: "${workspaceName}", expiry: ${formattedExpiry}`
  );
  ...
  try {
    const info = await transporter.sendMail({
      from: `"EasyTechnoMed Billing" <${smtpUser}>`,
      to: email,
      subject: `Subscription Plan Renewed for ${workspaceName || "Your Workspace"} - EasyTechnoMed`,
      html,
    });
    console.log(`[MAIL_DEBUG] Plan renewal email sent to ${email}. ID: ${info.messageId}`);
    return info;
  } catch (err) {
    console.error(`[MAIL_DEBUG_ERROR] Plan renewal email failed for ${email}:`, err);
    throw err;
  }
}
